import requests
from datetime import datetime

# ✅ Source M3U URL
SOURCE_M3U = "https://raw.githubusercontent.com/BINOD-XD/Toffee-Auto-Update-Playlist/refs/heads/main/toffee_OTT_Navigator.m3u"

# ✅ Output files
OUTPUT_FILE = "Toffee.m3u"
LOG_FILE = "update_log.txt"

def log_message(msg: str):
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    with open(LOG_FILE, "a", encoding="utf-8") as log:
        log.write(f"[{timestamp}] {msg}\n")

def update_playlist():
    print("🚀 Fetching Toffee M3U playlist...")
    try:
        r = requests.get(SOURCE_M3U, timeout=30)
        r.raise_for_status()
        content = r.text.strip()
        print("✅ Successfully fetched playlist!")
    except Exception as e:
        error_msg = f"❌ Error fetching playlist: {e}"
        print(error_msg)
        log_message(error_msg)
        content = ""  # fallback blank
     
    # Always write a file to avoid git error
    if not content.startswith("#EXTM3U"):
        msg = "⚠️ Invalid playlist format — adding fallback header"
        print(msg)
        log_message(msg)
        content = "#EXTM3U\n# Playlist fetch failed or invalid.\n"

    with open(OUTPUT_FILE, "w", encoding="utf-8") as f:
        f.write(content)

    success = f"✅ Toffee.m3u updated successfully at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    print(success)
    log_message(success)


if __name__ == "__main__":
    update_playlist()
